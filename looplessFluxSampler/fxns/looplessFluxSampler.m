...
%                                    * TICs - list of TICs in the model (obtained using ThermOptEnumMILP)
%                                    * Direction -  list of the TIC direction in the model (obtained using
%                                                                                          ThermOptEnumMILP)
...
%
% - Pavan Kumar Added flux sampling using TOF
...
if (nargin<2)
    ...
    % loopcheck algorithm (default loopcheck is TOF)
    if isfield(options,'loopcheck'); sample.loopcheck = options.loopcheck;
    else sample.loopcheck = 'TOF'; end
...
if sample.loopless
    ...
    % Define loopless feasibility 
    if strcmp(sample.loopcheck,'TOF')
        sample.TICs = tempModel.TICs;
        sample.Direction = tempModel.Direction;
        sample = getTICmat(sample);
        sample.isFeasible=@(points) looplessCheckTOF(points,sample);
    elseif strcmp(sample.loopcheck,'LP')
        sample.isFeasible=@(points) looplessCheckLP(points,sample);
    elseif strcmp(sample.loopcheck,'NINT')
